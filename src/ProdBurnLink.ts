...
import {createContextButton, getBuffersFromList, parseProdName} from "./util";
...
export class ProdBurnLink implements Module {
	...
	run(allBuffers) {
		...
		buffers.forEach(buffer => {
			...
			if(!contextBar || !contextBar.children[0]){return;}
			
			// Make run only once
			if(contextBar.classList.contains(this.tag)){return;}
