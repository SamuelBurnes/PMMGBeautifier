...
export const dateYearFormatter2 = new Intl.DateTimeFormat(undefined, {month: "2-digit", day: "2-digit", year: "2-digit"})
...
export function createContextButtonRow(buttonAbbreviations, buttonLabels, buttonLinks)
{
	...
	for(var i = 0; i < buttonAbbreviations.length; i++)
	{
...
export function parseProdName(text)
{
	try
	{
		...
		var match = text.match(/([A-Z]{2}-[0-9]{3} [a-z]) Production/);	// Unnamed system unnamed planet
		if(match && match[1])
		{
			return match[1].replace(" ","");
		}
...
*/
export function createMaterialElement(ticker, className: string = "prun-remove-js", amount: string = "none", text: boolean = false, small: boolean = false, building?)
{
	if(!MaterialNames[ticker] && ticker != "SHPT" && !building){return null;}	// Return nothing if the material isn't recognized
