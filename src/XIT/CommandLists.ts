...
function dispStoredList(result, param)
{
	...
	if(result["PMMG-Lists"][listName] && result["PMMG-Lists"][listName].length > 0)
	{
		result["PMMG-Lists"][listName].forEach(linkInfo => {
			if(!linkInfo[0] || !linkInfo[1]){return;}
			var line = document.createElement("tr");
			const textColumn = document.createElement("td");
...
function createEditInterface(tile, result, parameters, settings: any[] = [], listObj)
{
	...
	if(prefilled)
	{
		...
		{
			form.appendChild(createPopupInputRow("Link " + (i + 1) + " Label", settings[i][0], i == 0 ? "The label of the first link." : ""));
			form.appendChild(createPopupInputRow("Link " + (i + 1) + " Command", settings[i][1], i == 0 ? "The command opened by the first link (ex: CX NC1)" : ""));
		}
	}
	else
	{
