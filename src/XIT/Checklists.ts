...
function updatePopupInfo(junk, params)
{
	...
	case "Repair":
		...
		break;
	default:
		console.log("Popup: Invalid Selection Type")
	}
	...
	case "Repair":
		...
		if (site.buildings) {
			site.buildings.forEach(
				building =>
				{
					...
					{
						building.repairMaterials.forEach(
							mat =>
							{
		...
		Object.keys(mats).forEach(
			mat =>
			{
				...
				child.name = mats[mat].toLocaleString(
				undefined,
				{
					maximumFractionDigits: 0
				}) + " [[m:" + mat + "]]";
...
class Checklist // An individual checklist
{
	...
	recreateCallback(result, thisObject)
	{
		...
		{
			result["PMMG-Checklists"][thisObject.name].forEach(
				checkInfo =>
				{
	...
	constructor(checkInfo, checklist, isChild )
	{
		...
		if (checkInfo.duedate) // The due date under the name
		{
			...
			if (checkInfo.recurring) {
				dateText += " (every " +
					(checkInfo.recurring).toLocaleString(
						undefined,
						{
		...
		this.checkCircle.addEventListener(
			'click', 
			function () 
			{
					...
					{ // First, check whether the object is still completed (in local storage)
						...
						if (completed)
						{
							...
							else if (thisObject.checkInfo.recurring && thisObject.checkInfo.duedate) // Recur
							{
								...
								{
									thisObject.checkInfo.children.forEach(
										child =>
										{
