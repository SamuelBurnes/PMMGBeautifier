...
import {comparePlanets} from "../../GameProperties";
import {Style} from "../../Style";

// All functions associated with generating/editing action packages
export async function createGenerateScreen(tile, packageName, userInfo)
{
...
class GenerateScreen {
	...
	*/
	...
	private generateActionForm()
	{
			...
			*/
			...
			switch(action.type)
			{
				...
				case "MTRA":
					...
					{
						const origin = action.origin == "Configure on Execution" ? "configured location" : action.origin;
						const dest = action.dest == "Configure on Execution" ? "configured location" : action.dest;
						
						contentColumn.appendChild(createTextSpan("Transfer group " + action.group + " from " + origin + " to " + dest));
					}
					else
					{
						contentColumn.appendChild(createTextSpan("--"));
		...
		// Dropdown for adding new groups
		const newActionDropdown = this.createFormRow(this.actionSection, "select", "active", "Action Type", "actionType", undefined, ["CX Buy", "MTRA"]);
	...
	private createActionPopup(actionIndex)
	{
		...
		switch(action.type)
		{
			...
			case "MTRA":
				...
				const originNames = [...this.userInfo["PMMG-User-Info"].storage].sort(storageSort).map(parseStorageName) as any[];
				...
				const destNames = [...this.userInfo["PMMG-User-Info"].storage].sort(storageSort).map(parseStorageName) as any[];
