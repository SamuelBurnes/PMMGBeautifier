...
export function Settings(tile, parameters, oldResult, userInfo, webData, modules)
{
	clearChildren(tile);
	...
	getLocalStorage("PMMGExtended", function(result){
		...
		colorSelect.addEventListener("change", function(){
			getLocalStorage("PMMGExtended", function(updatedResult){
				updatedResult["PMMGExtended"]["color_scheme"] = colorSelect.selectedOptions[0].value || undefined;
				setSettings(updatedResult);
			});
		});
		...
		finCheckbox.addEventListener("click", function() {
			getLocalStorage("PMMGExtended", function(updatedResult){
				updatedResult["PMMGExtended"]["recording_financials"] = finCheckbox.checked;
				setSettings(updatedResult);
			});
		});
		...
		minCheckbox.addEventListener("click", function() {
			getLocalStorage("PMMGExtended", function(updatedResult)
			{
				updatedResult["PMMGExtended"]["minimize_by_default"] = minCheckbox.checked;
		...
		redIn.addEventListener("input", function(){
			getLocalStorage("PMMGExtended", function(updatedResult){
				updatedResult["PMMGExtended"]["burn_thresholds"][0] = parseFloat(redIn.value);
				setSettings(updatedResult);
			});
		});
		...
		yelIn.addEventListener("input", function(){
			getLocalStorage("PMMGExtended", function(updatedResult){
				updatedResult["PMMGExtended"]["burn_thresholds"][1] = parseFloat(yelIn.value);
				setSettings(updatedResult);
			});
		});
		...
		result["PMMGExtended"]["sidebar"].forEach(hotkey => {
			const div = createInputPair(hotkey, hotkeyInputDiv);
			if(div != null){hotkeyInputDiv.appendChild(div);}
			return;
		});
		
		const addButton = makePushButton("+", function(){
			const div = createInputPair([[],[]], hotkeyInputDiv);
			if(div != null){hotkeyInputDiv.appendChild(div);}
		}, Style.ButtonSuccess);
		...
		usernameInput.addEventListener("input", function(){
			getLocalStorage("PMMGExtended", function(updatedResult){
				updatedResult["PMMGExtended"]["username"] = !usernameInput.value || usernameInput.value == "" ? undefined : usernameInput.value;
				setSettings(updatedResult);
			});
		});
		...
		apiInput.addEventListener("input", function(){
			getLocalStorage("PMMGExtended", function(updatedResult){
				updatedResult["PMMGExtended"]["apikey"] = !apiInput.value || apiInput.value == "" ? undefined : apiInput.value;
				setSettings(updatedResult);
			});
		});
		...
		exclusionInput.addEventListener("input", function(){
			getLocalStorage("PMMGExtended", function(updatedResult){
				updatedResult["PMMGExtended"]["unpack_exceptions"] = exclusionInput.value.split(",");
				setSettings(updatedResult);
			});
		});
		...
		modules.forEach(mp => {
			...
			const toggle = makeToggleButton("On", "Off", () => {
				getLocalStorage("PMMGExtended", function(updatedResult){
					...
					if(updatedResult["PMMGExtended"]["disabled"].includes(mp.name))
					{
						if(mp.enabled){
							for(var i = 0; i < updatedResult["PMMGExtended"]["disabled"].length; i++)
							{
								if(updatedResult["PMMGExtended"]["disabled"][i] == mp.name)
								{
									updatedResult["PMMGExtended"]["disabled"].splice(i, 1);
									i--;
								}
							}
					...
					{
						if(!mp.enabled){updatedResult["PMMGExtended"]["disabled"].push(mp.name);}	// Was just disabled, add disabled label
					}
					setSettings(updatedResult);
				});
			}, mp.enabled);
	...
	return [parameters, webData, userInfo, oldResult];
}
...
function createInputPair(hotkey, fullDiv)
{
	...
	displayedValue.addEventListener("input", function(){
		...
		getLocalStorage("PMMGExtended", function(updatedResult){
			updatedResult["PMMGExtended"]["sidebar"] = hotkeys;
			setSettings(updatedResult);
		});
	});
